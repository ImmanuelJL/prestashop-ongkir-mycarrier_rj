...
class AddressControllerCore extends FrontController
{
     ...
     * Assign template vars related to countries display
     */
    protected function assignCity()
    {
        // @todo use helper
        $responseCity = file_get_contents("modules/mycarrier_rj/controllers/front/city-ojb.json", FILE_USE_INCLUDE_PATH);
        $responseCity = json_decode($responseCity);
        $list = null;

        if( isset($responseCity) && isset($responseCity->rajaongkir) && isset($responseCity->rajaongkir->results) && isset($this->context) && isset($this->context->cart) && isset($this->context->cart->id_address_delivery) && isset($this->context) && isset($this->context->cookie) && isset($this->context->cookie->id_lang) ){
            ...
            foreach ($responseCity->rajaongkir->results as $city) {
                $selected = ((string)$city->city_name === $address->city) ? ' selected="selected"' : '';
                $list .= '<option value="'.$city->city_name.'"'.$selected.'>'.htmlentities($city->city_name, ENT_COMPAT, 'UTF-8').'</option>';
            }
        }
